...
```{R}
integrated <- readRDS( here('data','Tcell_recluster.RDS'))

data.input = GetAssayData(integrated@assays$SCT)# normalized data matrix ; Question: I think this is our normalized data via SCT transform
meta = integrated@meta.data # a dataframe with rownames containing cell mata data
...
```{r}
meta$labels = as.numeric(meta$seurat_clusters) +1
...
## cellchat@netP$pathways ; "MIF"  "PARs" "CD40"
pathways.show <- c("MIF") 
# Hierarchy plot
...
# Chord diagram
group.cellType <- c(rep("MIF", 4), rep("PARs", 4), rep("CD40", 4)) # grouping cell clusters into MIF, PARs and CD40 cells
...
netVisual_chord_gene(cellchat, sources.use = 4, targets.use = c(2:7), lab.cex = 0.5,legend.pos.y = 30)
...
```{r, eval=FALSE}
plotGeneExpression(cellchat, signaling = "PARs", enriched.only = FALSE)
```
...
gg2 <- netAnalysis_signalingRole_scatter(cellchat, signaling = c("PARs", "CD40"))
gg1 + gg2
```
...
ht1 + ht2
# Signaling role analysis on the cell-cell communication networks of interest
ht <- netAnalysis_signalingRole_heatmap(cellchat, signaling = c("PARs", "CD40"))
```
